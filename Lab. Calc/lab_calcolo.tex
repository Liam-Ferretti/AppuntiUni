...
int main(){
	...
	\begin{itemize}
		\item Header: inclusione delle direttive, come le librerie ad esempio include<stdio.h>, o il DEFINE per definire le costanti simboliche, in questo modo ogni volta che nel programma si incontra la costante, verrà sostituita con il suo valore.
		
		Nell'header si dichiarano anche le funzioni che verranno poi usate nel file.
		\item Body: ovvero la parte del programma compresa tra le graffe dell'int main
		\item Funzioni
	\end{itemize}
	
	Si possono compilare più file, concatenandoli in successione prima di "-o" all'interno del comando per compilare, e produce un singolo eseguibile.
...
int main(){
	...
	\end{code}
	Bisogna specificare il tipo ed il nome della variabile, esistono diverse tipologie di variabili:
	\begin{itemize}
		\item \textbf{int}: intero (16bit)
		\item \textbf{float}, \textbf{double}: numero razionale, in virgola mobile (32bit, 64bit)
		\item \textbf{char}, singolo carattere (8bit)
		\item Classificatori:
		\subitem short
		\subitem long
	\end{itemize}
	
	\subsection{Assegnazione delle variabili}
	\begin{code}
int main(){
	...
	//il segno = è l'operatore di assegnazione, assegna ala variabile a sinistra l'elemento a destra, è comune assegnare 0 alla variabile in quanto gli spazi di memoria potrebbero essere già stati usati
	...
	\begin{table}[h!]
		...
		\begin{tabular}{|c|c|}
			\hline
			\textbf{Operazione} & \textbf{Simbolo} \\
			\hline
			Assegnazione & = \\
			\hline
			Somma & + \\
			...
			\hline
		\end{tabular}
		\caption{Operatori aritmetici e di assegnazione}
	\end{table}
	
	\section{Stringhe di formato}
	\begin{itemize}
		\item \textbf{\%d}, \textbf{\%i}, \textbf{\%x} — Interi (\texttt{int})  
		Esempio: \texttt{printf("Valore: \%d", 10);}
		
		\item \textbf{\%f}, \textbf{\%lf}, \textbf{\%g}, \textbf{\%e} — Numeri reali (\texttt{float} / \texttt{double})  
		Esempio: \texttt{printf("Pi: \%f", 3.14);}
		
		\item \textbf{\%c} — Caratteri (\texttt{char})  
		Esempio: \texttt{printf("Lettera: \%c", 'A');}
		
		\item \textbf{\%s} — Stringhe (\texttt{string})  
		Esempio: \texttt{printf("Stringa: \%s", "Ciao");}
